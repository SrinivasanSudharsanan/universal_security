...
Tests Roles, RLS, and Column Security on actual business data
"""

from engines.unified_connector import UnifiedConnector
from config import get_settings

class RealDataSecurityTester:
    def __init__(self):
        self.connector = UnifiedConnector()
        self.settings = get_settings()
        self.current_user_role = None
        
        # Security policies for real business data
        self.security_policies = {
            ...
            'rls_policies': {
                ...
                'sales': {
                    ...
                    'sales_rep': "amount < 6000",  # Smaller deals only
            ...
            'cls_policies': {
                'employees': {
                    'admin': ['name', 'department', 'salary', 'access_level'],
                    'hr': ['name', 'department', 'salary'],  # No access_level
                    'engineering_manager': ['name', 'department', 'access_level'],  # No salary
                    'sales_manager': ['name', 'department', 'access_level'],  # No salary
                    'employee': ['name', 'department'],  # Minimal info
                    'public': ['name', 'department']  # Public info only
                },
                'sales': {
                    'admin': ['product', 'amount', 'customer', 'region', 'quarter'],
                    'sales_director': ['product', 'amount', 'customer', 'region', 'quarter'],
                    'regional_manager': ['product', 'amount', 'region', 'quarter'],  # No customer names
                    'sales_rep': ['product', 'amount', 'quarter'],  # Limited info
